...
// ── Gratis live-demo: upload → AI-resultat med vandmærke (ingen konto) ───────
const DEMO_ROOMS = [
...
function LiveDemoSection() {
  ...
  const pickFile = (f: File | null) => {
    if (!f) return;
    if (!f.type.startsWith("image/")) { setErrorMsg("Vælg venligst en billedfil (JPG eller PNG)."); return; }
    if (f.size > 10 * 1024 * 1024) { setErrorMsg("Billedet må højst fylde 10 MB."); return; }
    setErrorMsg(null);
    setFile(f);
  ...
  const startProgress = () => {
    ...
    progressTimer.current = setInterval(() => {
      setProgress((p) => (p < 90 ? p + Math.max(0.4, (90 - p) / 45) : p));
    }, 600);
  };
  const stopProgress = () => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      fd.append("room", room);
      const res = await fetch("/api/bolig/demo-generate", { method: "POST", body: fd });
      const json = await res.json().catch(() => ({}));
      stopProgress();
      if (res.status === 429) {
        setRateLimited(true);
        setErrorMsg(json.message || "Dagens gratis prøve er brugt.");
        setPhase("error");
        return;
      }
      if (!res.ok || !json.success || !json.image_url) {
        setErrorMsg(json.message || "Generering mislykkedes — prøv igen om lidt.");
        setPhase("error");
        return;
      }
      setProgress(100);
      setResult({ before: json.original_url, after: json.image_url });
      setPhase("done");
    } catch {
      stopProgress();
  ...
  return (
    <section id="prov-selv" style={{ background: C.white, paddingTop: "clamp(52px, 8vw, 100px)", paddingBottom: "clamp(52px, 8vw, 100px)" }} className="px-4 sm:px-6" data-testid="bolig-live-demo">
      <div className="mx-auto max-w-3xl">
        <div className="text-center mb-10">
          <Overline>Prøv det selv — uden konto</Overline>
          <H2>Se dit eget rum forvandlet på ét minut</H2>
          <p className="mt-4 max-w-xl mx-auto" style={{ color: C.muted, fontSize: 16, lineHeight: 1.6 }}>
            Upload et foto af et rum og få en gratis AI-visualisering i skandinavisk stil. Ingen betalingskort, ingen oprettelse.
          </p>
        </div>

        {phase === "done" && result ? (
          <div>
            <BeforeAfterSlider beforeSrc={result.before} afterSrc={result.after} className="bg-white shadow-lg" />
            <div className="mt-8 text-center" style={{ background: C.navy, borderRadius: 12, padding: "clamp(24px, 4vw, 36px)" }} data-testid="demo-result-cta">
              <div style={{ fontFamily: SERIF, color: C.white, fontSize: "clamp(19px, 3vw, 24px)", fontWeight: 500 }}>
                Vil du prøve med flere rum og stilarter?
              </div>
              <p className="mt-2 mb-6 mx-auto" style={{ color: "rgba(255,255,255,0.75)", fontSize: 14, maxWidth: 420, lineHeight: 1.6 }}>
                Opret en gratis konto og få 2 visualiseringer mere — uden vandmærke på tværs af 8 stilarter.
              </p>
              <Link href="/opret" data-testid="link-demo-signup">
                <span style={{ background: C.gold, color: C.navy, padding: "13px 26px", borderRadius: 8, fontWeight: 700, fontSize: 15, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 8 }}>
                  Opret gratis konto <ArrowRight size={16} />
                </span>
              </Link>
              <button onClick={reset} className="block mx-auto mt-4" style={{ color: "rgba(255,255,255,0.55)", fontSize: 13, textDecoration: "underline", background: "none", border: "none", cursor: "pointer" }} data-testid="button-demo-reset">
                Prøv med et andet foto
              </button>
            </div>
          </div>
        ) : (
          <div style={{ background: C.warm, border: `1px solid ${C.champagne}`, borderRadius: 12, padding: "clamp(20px, 4vw, 32px)" }}>
            <input
              ref={fileInputRef}
              type="file"
            ...
            {!previewUrl ? (
              <button
                ...
                style={{ border: `2px dashed ${C.goldBorder}`, borderRadius: 10, padding: "clamp(36px, 6vw, 64px) 16px", background: C.white, cursor: "pointer" }}
                data-testid="button-demo-upload"
              >
                <div className="flex items-center justify-center mb-4" style={{ width: 56, height: 56, borderRadius: "50%", background: C.goldTint }}>
                  <Upload className="w-6 h-6" style={{ color: C.gold }} />
                </div>
                <div style={{ color: C.navy, fontSize: 16, fontWeight: 600 }}>Upload et rumfoto</div>
                <div className="mt-1" style={{ color: C.muted, fontSize: 13 }}>Klik her eller træk et billede ind — JPG/PNG, maks. 10 MB</div>
              </button>
            ) : (
              <div>
                <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", background: "#000" }}>
                  <img src={previewUrl} alt="Dit rumfoto" style={{ width: "100%", maxHeight: 420, objectFit: "contain", display: "block", opacity: phase === "generating" ? 0.55 : 1, transition: "opacity 0.3s" }} data-testid="img-demo-preview" />
                  {phase === "generating" && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center" data-testid="demo-generating-overlay">
                      <Loader2 className="w-8 h-8 animate-spin" style={{ color: C.gold }} />
                      <div className="mt-3" style={{ color: C.white, fontSize: 14, fontWeight: 600 }}>AI'en arbejder…</div>
                      <div className="mt-1" style={{ color: "rgba(255,255,255,0.7)", fontSize: 12 }}>Tager typisk 30–60 sekunder</div>
                      <div className="mt-4" style={{ width: "60%", maxWidth: 260, height: 4, background: "rgba(255,255,255,0.2)", borderRadius: 2, overflow: "hidden" }}>
                        <div style={{ width: `${progress}%`, height: "100%", background: C.gold, transition: "width 0.6s" }} />
                      </div>
                    </div>
                  )}
                  {phase !== "generating" && (
                    <button
                      ...
                      style={{ background: "rgba(15,25,35,0.75)", color: C.white, fontSize: 12, padding: "7px 14px", borderRadius: 999, border: "none", cursor: "pointer" }}
                      data-testid="button-demo-change-photo"
                    >
                      Skift foto
                    </button>
                  )}
                ...
                <div className="mt-5">
                  <div style={{ color: C.navy, fontSize: 13, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>Hvilket rum er det?</div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {DEMO_ROOMS.map((r) => (
                      <button
                        key={r.value}
                        onClick={() => phase !== "generating" && setRoom(r.value)}
                        style={{
                          padding: "9px 18px",
                          borderRadius: 999,
                          fontSize: 14,
                          ...
                          border: `1px solid ${room === r.value ? C.gold : C.champagne}`,
                          background: room === r.value ? C.gold : C.white,
                          color: room === r.value ? C.navy : C.muted,
                          transition: "all 0.15s",
                        }}
                        data-testid={`button-demo-room-${r.value.replace(/\s/g, "-")}`}
                      >
                        {r.label}
                      </button>
                    ))}
                ...
                <button
                  ...
                  style={{
                    background: phase === "generating" ? "rgba(15,25,35,0.55)" : C.navy,
                    color: C.white,
                    padding: "16px 24px",
                    borderRadius: 8,
                    fontSize: 16,
                ...
                >
                  {phase === "generating" ? (<><Loader2 className="w-5 h-5 animate-spin" /> Genererer…</>) : (<>Se rummet forvandlet — gratis <ArrowRight size={18} /></>)}
                </button>
              </div>
            )}

            {errorMsg && (
              <div className="mt-4 text-center" style={{ background: rateLimited ? C.goldTint : "rgba(180, 60, 60, 0.08)", border: `1px solid ${rateLimited ? C.goldBorder : "rgba(180,60,60,0.25)"}`, borderRadius: 8, padding: "14px 16px" }} data-testid="text-demo-error">
                <div style={{ color: rateLimited ? C.navy : "#A03B3B", fontSize: 14, fontWeight: 500 }}>{errorMsg}</div>
                {rateLimited && (
                  <Link href="/opret" data-testid="link-demo-ratelimit-signup">
                    <span className="inline-flex items-center gap-1.5 mt-3" style={{ background: C.navy, color: C.white, padding: "10px 20px", borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
                      Opret gratis konto <ArrowRight size={14} />
                    </span>
                  </Link>
                )}
              </div>
            ...
            <p className="mt-4 text-center" style={{ color: C.muted, fontSize: 12 }}>
              1 gratis prøve pr. dag · Resultatet får et Forma Estates-vandmærke · Billedet bruges kun til din visualisering
            </p>
          </div>
        )}
      </div>
