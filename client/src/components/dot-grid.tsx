...
// Inline styles (replaces DotGrid.css)
const styles: Record<string, CSSProperties> = {
  section: { display: "flex", alignItems: "center", justifyContent: "center", height: "100%", width: "100%", position: "relative" },
...
const throttle = <T extends unknown[]>(func: (...args: T) => void, limit: number) => {
  ...
  return function (...args: T) {
    ...
    if (now - lastCall >= limit) { lastCall = now; func(...args); }
  };
};
...
const DotGrid = ({
  ...
  useEffect(() => {
    ...
    return () => { if (ro) ro.disconnect(); else globalThis.removeEventListener("resize", buildGrid); };
  }, [buildGrid]);
  ...
  return (
    <section className={className} style={{ ...styles.section, ...style }}>
