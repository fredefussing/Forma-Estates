...
- **Purchase Fulfillment Ledger**: All paid purchases (Stripe subscriptions, Stripe one-time packages, Shopify credit packages) are recorded in a `pending_purchases` table keyed by a unique external id (`stripe:<sessionId>` / `shopify:<orderId>`). Fulfillment happens exclusively through `server/purchases.ts` `claimAndGrant()`, which claims + grants + writes the ledger row in ONE database transaction — guaranteeing a purchase is granted exactly once (across accounts, devices, webhook retries and success-page races). Purchases made before signup auto-claim at account creation in `/api/auth/verify`. One-time packages add quotas on top WITHOUT a reset date (never renew); the monthly reset restores only the subscription tier's base and carries over unused top-ups. Webhooks are verified: Stripe requires `STRIPE_WEBHOOK_SECRET` in production, Shopify verifies `X-Shopify-Hmac-Sha256` when `SHOPIFY_WEBHOOK_SECRET` is set. Regression suite: `npx tsx scripts/test-purchase-flows.ts` (40 assertions, no AI calls).
